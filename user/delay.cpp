...
__fast_inline UQ4_28 lookUpFbGain(float depth)
{
  if ( 1.f <= depth ) {
    return fbGainTable[LCW_DELAY_FB_GAIN_TABLE_SIZE - 1];
  }
...
{
  const float delaySize = fasterpow2f( 12.f + ((15.f - 12.f) * s_time) );
  LCWDelayUpdate( (uint32_t)delaySize );
  ...
  const float fb = lookUpFbGain(s_depth) / (float)(1 << 28);
  ...
  for (; x != x_e; ) {
    ...
    LCWDelayInput(
      (SQ7_24)((xL * s_inputGain) * (1 << 24)),
      LCWDelayReverse() - (SQ7_24)((wL * fb) * (1 << 24)) );
